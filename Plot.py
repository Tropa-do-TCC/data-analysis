...
class Plot:
    ...
    def distance(self, x_point, y_point, z_point, event):
        ...
        x2, y2, _ = proj3d.proj_transform(x_point, y_point, z_point, plt.gca().get_proj())
    ...
    def calcClosestDatapoint(self, event):
        ...
        distances = [self.distance (self.x[i], self.y[i], self.z[i], event) for i in range(len(self.x))]
    ...
    def annotatePlot(self, index):
        ...
        x2, y2, _ = proj3d.proj_transform(self.x[index], self.y[index], self.z[index], self.ax.get_proj())
